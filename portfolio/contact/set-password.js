...
//   const { user } = useSelector((state) => state.user);
  ...
  useEffect(() => {
    ...
    if (user && user.password) {
      setLoading(true);
      const x = async () => {
        const encryptedEmail =await AES.encrypt(user?.email, "mysecretKeyErnest").toString();
