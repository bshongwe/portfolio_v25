...
const Login = () => {
 ...
 const registerSubmit = async (e) => {
   ...
   try {
     ...
     const result = await signIn("credentials", {
      ...
      if (rememberMe) {
        const encryptedEmail =await AES.encrypt(email, "mysecretKeyErnest").toString();
        const encryptedPassword =await AES.encrypt(password, "mysecretKeyErnest").toString();
 ...
 useEffect(() => {
   ...
   if (encryptedEmail && encryptedPassword) {
     const decryptedEmail = AES.decrypt(encryptedEmail, "mysecretKeyErnest").toString(
       Utf8
     );
